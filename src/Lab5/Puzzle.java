		...
		for(int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				if(currentState.getTile(i, j)!=goalState.getTile(i, j) && currentState.getTile(i, j)!=0) {
	                ...
	                int targetTile = goalState.getTile(i, j);
	                int[] currentLocation = currentState.getLocation(currentTile);
	                int[] targetLocation = goalState.getLocation(targetTile);
	                int manhattanDistance = Math.abs(targetLocation[0] - currentLocation[0]) + Math.abs(targetLocation[1] - currentLocation[1]);
